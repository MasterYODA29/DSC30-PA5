 ...
 */
 ...
 public class MNIST {
     ...
     public static float totalDist(float[] img1, float[] img2) throws IllegalArgumentException {
         ...
         {
            sqrSum+=Math.pow(img1[i]-img2[i],2);
         }
         float finAns=(float) Math.pow(sqrSum,0.5);
         return finAns;
     }
     ...
     public static DataHolder[] getClosestMatches(float[] image, int k) {
         // TODO
         dHeap<DataHolder> pQueue=new dHeap<>(2,NUM_TRAIN,false);
         for (int i=0;i<NUM_TRAIN;i++)
         {
            float priority=totalDist(image, TRAIN_IMAGES[i]);
            pQueue.add(new DataHolder(TRAIN_LABELS[i], priority, TRAIN_IMAGES[i]));
         }

         DataHolder[] finalQueue=new DataHolder[k];
         for (int i=0;i<k;i++)
         {
            DataHolder ele=pQueue.remove();
            finalQueue[i]=ele;
         }
         return finalQueue;
     }
     ...
     public static int predict(DataHolder[] closestMatches) {
        ...
        for (DataHolder d: closestMatches)
        {
         count[d.label]+=1;
        }
        int max=count[0];
